...
class Cmall extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Cmall_category','Search_keyword');
    ...
    function __construct()
    {
        ...
        if ( ! $this->cmallconfig->item('use_cmall')) {
            alert('이 웹사이트는 ' . html_escape($this->cmallconfig->item('cmall_name')) . ' 기능을 사용하지 않습니다');
            return;
        }
    ...
    public function index()
    {
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall');
        $page_name = $this->cmallconfig->item('site_page_name_cmall');

        $searchconfig = array(
            '{컨텐츠몰명}',
        );
        $replaceconfig = array(
            $this->cmallconfig->item('cmall_name'),
        );
...
//       인기검색어 끝
        ...
        $layoutconfig = array(
            ...
            'layout' => 'layout',
            'skin' => 'cmall',
            'layout_dir' => $this->cmallconfig->item('layout_cmall'),
            'mobile_layout_dir' => $this->cmallconfig->item('mobile_layout_cmall'),
            'use_sidebar' => $this->cmallconfig->item('sidebar_cmall'),
            'use_mobile_sidebar' => $this->cmallconfig->item('mobile_sidebar_cmall'),
            'skin_dir' => $this->cmallconfig->item('skin_cmall'),
            'mobile_skin_dir' => $this->cmallconfig->item('mobile_skin_cmall'),
    ...
    public function get_image_url($method)
    {
//        $this->load->library("cimongo/cimongo");
//        $mongid= new MongoId( $method);
//        $result_sqlaaa = $this->cimongo->get_where("store_image_info",array("_id" => $mongid));
//        $row = $result_sqlaaa->result_array();
//        echo $row;
//        return $row[0]['store_info'][0][1];
    }
    ...
    public function lists($category_id = '')
    {
        ...
        $access_list = $this->cmallconfig->item('access_cmall_list');
        $access_list_level = $this->cmallconfig->item('access_cmall_list_level');
        $access_list_group = $this->cmallconfig->item('access_cmall_list_group');
...
//        echo $result_sub;
//        $list_store_cnt =
//        $result_sqlaaa = $this->cimongo->get_where("store_image_info",array("store_name" => '치즈팩토리카페'));
//        $this->mongoq->limit(10);
//        $row = $result_sqlaaa->result_array();
//        echo $row[0]['store_info'][0][1];
//        echo 1;
...
//            foreach ( element('list', $result) as $key => $val) {
                ...
                if($key < 16){
                    $result['list'][$key]['cit_key'] =element('Name', $val);
//                    $mongid= new MongoId( $val['_id']->{'$id'});
                    $result_sqlaaa = $this->cimongo->get_where("store_image_info",array("store_name" => element('Name', $val)));

                    $row = $result_sqlaaa->result_array();
                    $result['list'][$key]['cit_file_1']=$row[0]['store_info'][0][1];
//                    $result['list'][$key]['cit_file_open']=file_get_contents($result['list'][$key]['cit_file_1']);
...
//                $result['list'][$key]['category'] = $this->Cmall_category_model->get_category(element('cit_id', $val));
        ...
        $result = $this->Search_keyword_model->get_admin_list('', '', '', '', '', '', '', '');
        $view['view']['fch_data'] = $result;
...
//       인기검색어 끝
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_list');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_list');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_list');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_list');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_list');
    ...
    public function item($cit_key = '1')
    {
        // 이벤트 라이브러리를 로딩합니다
...
//        $data['meta'] = $this->Cmall_item_meta_model->get_all_meta(element('cit_id', $data));
        $result_sql = $this->cimongo->get_where("store",array('Name'=>'양군'));
        $result_array = $result_sql->result_array();
        foreach ( $result_array as $key => $val) {
            ...
            $result_sqlaaa = $this->cimongo->get_where("store_image_info",array("store_name" => element('Name', $val)));
            $row = $result_sqlaaa->result_array();
            $data['cit_file_1']=$row[0]['store_info'][0][1];
//                    $result['list'][$key]['cit_file_open']=file_get_contents($result['list'][$key]['cit_file_1']);
            $data['cit_name'] =element('Name', $val);
            $data['cit_img_src'] =element('hasgtag', $val);
            $data['category'] = element('category', $val);
            $data['collect_region'] = element('collect_region', $val);
            $data['phonenumber'] = element('phonenumber', $val);
            $data['new_address'] = element('new_address', $val);
            $data['index'] = element('index', $val);
        }
//        $data['detail']['detail'] = $this->Cmall_item_detail_model->get_all_detail(element('cit_id', $data));
/*
        //카테고리 표시시
       $data['category'] = $this->Cmall_category_model->get_category(element('cit_id', $data));
        $alertmessage = $this->member->is_member()
            ? '회원님은 상품 페이지를 볼 수 있는 권한이 없습니다'
            : '비회원은 상품 페이지를 볼 수 있는 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오';
        $access_read = $this->cmallconfig->item('access_cmall_read');
        $access_read_level = $this->cmallconfig->item('access_cmall_read_level');
        $access_read_group = $this->cmallconfig->item('access_cmall_read_group');
        $this->accesslevel->check(
            $access_read,
            $access_read_level,
            $access_read_group,
        ...
        $thumb_width = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('cmall_product_mobile_thumb_width')
            : $this->cmallconfig->item('cmall_product_thumb_width');
        $autolink = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('use_cmall_product_mobile_auto_url')
            : $this->cmallconfig->item('use_cmall_product_auto_url');
        $popup = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('cmall_product_mobile_content_target_blank')
            : $this->cmallconfig->item('cmall_product_content_target_blank');
...
*/
        ...
        $layout_dir = (element('item_layout', element('meta', $data))) ? element('item_layout', element('meta', $data)) : $this->cmallconfig->item('layout_cmall');
        $mobile_layout_dir = (element('item_mobile_layout', element('meta', $data))) ? element('item_mobile_layout', element('meta', $data)) : $this->cmallconfig->item('mobile_layout_cmall');
        $use_sidebar = (element('item_layout', element('meta', $data))) ? element('item_sidebar', element('meta', $data)) : $this->cmallconfig->item('sidebar_cmall');
        $use_mobile_sidebar = (element('item_mobile_layout', element('meta', $data))) ? element('item_mobile_sidebar', element('meta', $data)) : $this->cmallconfig->item('mobile_sidebar_cmall');
        $skin_dir = (element('item_skin', element('meta', $data))) ? element('item_skin', element('meta', $data)) : $this->cmallconfig->item('skin_cmall');
        $mobile_skin_dir = (element('item_mobile_skin', element('meta', $data))) ? element('item_mobile_skin', element('meta', $data)) : $this->cmallconfig->item('mobile_skin_cmall');
        ...
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_item');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_item');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_item');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_item');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_item');
...
//       인기검색어 끝
    ...
    public function cartoption()
    {
        ...
        $skindir = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('mobile_skin_cmall')
            : $this->cmallconfig->item('skin_cmall');
        if (empty($skindir)) {
            $skindir = ($this->cbconfig->get_device_view_type() === 'mobile')
    ...
    public function itemimage($cit_key = '')
    {
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = element('cit_name', $data) . ' > ' . $this->cmallconfig->item('cmall_name') . ' 이미지 크게보기';
        $layoutconfig = array(
            'path' => 'cmall',
            'layout' => 'layout_popup',
            'skin' => 'itemimage',
    ...
    public function cart()
    {
        ...
        if ( ! $result = $this->cache->get($cachename)) {
            $days = $this->cmallconfig->item('cmall_cart_keep_days')
                ? $this->cmallconfig->item('cmall_cart_keep_days') : 14;
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_cart');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_cart');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_cart');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_cart');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_cart');
    ...
    public function order()
    {
        ...
        $access_buy = $this->cmallconfig->item('access_cmall_buy');
        $access_buy_level = $this->cmallconfig->item('access_cmall_buy_level');
        $access_buy_group = $this->cmallconfig->item('access_cmall_buy_group');
        $this->accesslevel->check(
            $access_buy,
            $access_buy_level,
            $access_buy_group,
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_order');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_order');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_order');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_order');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_order');
    ...
    public function orderresult($cor_id = 0)
    {
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_orderresult');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_orderresult');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_orderresult');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_orderresult');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_orderresult');
    ...
    public function orderupdate($agent_type='')
    {
        ...
        if ( ! is_numeric($this->input->post('order_deposit'))) {
            alert(html_escape($this->depositconfig->item('deposit_name')) . ' 의 값은 숫자만 와야 합니다');
        }
        ...
        if ($order_deposit) {
            if ($order_deposit < 0) {
                alert(html_escape($this->depositconfig->item('deposit_name')) . ' 의 값은 0 보다 작을 수 없습니다 ', site_url('cmall/order'));
            }
            if ($order_deposit > $total_price_sum) {
                alert(html_escape($this->depositconfig->item('deposit_name')) . ' 의 값은 총 결제금액보다 클 수 없습니다', site_url('cmall/order'));
            }
            if ($order_deposit > (int) $this->member->item('total_deposit')) {
                alert(html_escape($this->depositconfig->item('deposit_name')) . ' 값이 회원님이 보유하고 계신 값보다 큰 값이 입력되어서 진행할 수 없습니다', site_url('cmall/order'));
            }
        }
    ...
    public function orderlist()
    {
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_orderlist');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_orderlist');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_orderlist');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_orderlist');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_orderlist');
    ...
    public function wishlist()
    {
         ...
         * 레이아웃을 정의합니다
         */
        $page_title = $this->cmallconfig->item('site_meta_title_cmall_wishlist');
        $meta_description = $this->cmallconfig->item('site_meta_description_cmall_wishlist');
        $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_wishlist');
        $meta_author = $this->cmallconfig->item('site_meta_author_cmall_wishlist');
        $page_name = $this->cmallconfig->item('site_page_name_cmall_wishlist');
    ...
    public function reviewlist($cit_id = 0)
    {
        ...
        $thumb_width = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('cmall_product_review_mobile_thumb_width')
            : $this->cmallconfig->item('cmall_product_review_thumb_width');
        $autolink = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('use_cmall_product_review_mobile_auto_url')
            : $this->cmallconfig->item('use_cmall_product_review_auto_url');
        $popup = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('cmall_product_review_mobile_content_target_blank')
            : $this->cmallconfig->item('cmall_product_review_content_target_blank');
    ...
    public function review_write($cit_id = 0, $cre_id = 0)
    {
         ...
         **/
        if ( ! $this->cmallconfig->item('use_cmall_product_review_anytime')) {
        ...
        if ($this->form_validation->run() === false) {
             ...
             * 레이아웃을 정의합니다
             */
            $page_title = $this->cmallconfig->item('site_meta_title_cmall_review_write');
            $meta_description = $this->cmallconfig->item('site_meta_description_cmall_review_write');
            $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_review_write');
            $meta_author = $this->cmallconfig->item('site_meta_author_cmall_review_write');
            $page_name = $this->cmallconfig->item('site_page_name_cmall_review_write');
            ...
            $content_type = $this->cmallconfig->item('use_cmall_product_review_dhtml') ? 1 : 0;
            $updatedata = array(
                'cit_id' => $cit_id,
            ...
            if ($cre_id) {
                ...
                if ( ! $this->cmallconfig->item('use_cmall_product_review_approve')) {
    ...
    public function qnalist($cit_id = 0)
    {
        ...
        $thumb_width = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('cmall_product_qna_mobile_thumb_width')
            : $this->cmallconfig->item('cmall_product_qna_thumb_width');
        $autolink = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('use_cmall_product_qna_mobile_auto_url')
            : $this->cmallconfig->item('use_cmall_product_qna_auto_url');
        $popup = ($this->cbconfig->get_device_view_type() === 'mobile')
            ? $this->cmallconfig->item('cmall_product_qna_mobile_content_target_blank')
            : $this->cmallconfig->item('cmall_product_qna_content_target_blank');
    ...
    public function qna_write($cit_id = 0, $cqa_id = 0)
    {
        ...
        if ($this->form_validation->run() === false) {
             ...
             * 레이아웃을 정의합니다
             */
            $page_title = $this->cmallconfig->item('site_meta_title_cmall_qna_write');
            $meta_description = $this->cmallconfig->item('site_meta_description_cmall_qna_write');
            $meta_keywords = $this->cmallconfig->item('site_meta_keywords_cmall_qna_write');
            $meta_author = $this->cmallconfig->item('site_meta_author_cmall_qna_write');
            $page_name = $this->cmallconfig->item('site_page_name_cmall_qna_write');
            ...
            $content_type = $this->cmallconfig->item('use_cmall_product_qna_dhtml') ? 1 : 0;
